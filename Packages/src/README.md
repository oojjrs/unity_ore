...
- `Ore.SendReportAsync`: 스크린샷과 `Player.log`를 자동 첨부해 보고서를 전송한다.
- `Ore.SendUxAsync`: UX 이벤트를 한 번의 호출로 전송한다.
- `ReporterClient.SendReportAsync`: 보고서 JSON과 첨부파일을 전송한다.
- `ReporterClient.SendEventAsync`: 구조화된 진단 및 UX 이벤트를 전송한다.
- `ReportAttachment`: 바이트, 텍스트, JPEG 또는 PNG 첨부파일을 구성한다.
- `ReporterException`: HTTP 상태 코드와 서버 응답 본문을 제공한다.
...
이 패키지는 서버 통신, 요청 데이터 구성, 응답 및 오류 반환을 담당한다. 게임별 신고 UI, 진행 표시, 사용자 알림, 프로필 직렬화는 소비 프로젝트가 담당한다.
