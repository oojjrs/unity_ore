# Changelog

## 1.2.4

- 보고서 ZIP에서 `report.json`을 제거하고 첨부파일을 ZIP 루트에 저장하도록 변경했습니다.

## 1.2.3

- 스크린샷 축소 시 원본 텍스처의 sRGB 플래그에 맞춰 RenderTexture 색공간을 선택하도록 수정했습니다.

## 1.2.2

- FHD 스크린샷 축소 텍스처를 `RGBA32` 포맷으로 생성하도록 변경했습니다.

## 1.2.1

- 스크린샷 캡처의 중간 JPEG 재변환을 제거하고, 보고서 첨부 JPEG 기본 품질을 90으로 변경했습니다.

## 1.2.0

- 게임 클라이언트의 매치 검증 바이트 배열을 ZIP으로 압축해 전송하는 API를 추가했습니다.

## 1.1.6

- 전체 `Player.log`를 포함한 보고서 JSON과 첨부파일을 `report.zip` 하나로 압축해 전송하도록 변경했습니다.

## 1.1.5

- FHD 범위를 초과하는 스크린샷만 종횡비를 유지해 축소하고 JPEG `Texture2D`로 반환하는 `Ore.GetScreenshotAsJpeg`를 추가했습니다.

## 1.1.4

- 미지정 발생 시각을 JSON에서 생략하고, 전송 실패 로그에 서버 응답 본문을 표시하도록 수정했습니다.

## 1.1.3

- 비어 있거나 JSON `null`인 컨텍스트와 이벤트 속성을 빈 객체로 전송하고, 미지정 발생 시각은 nullable 날짜로 유지하도록 수정했습니다.

## 1.1.2

- 이벤트, 보고서, 설정, 첨부파일의 `null` 문자열 입력을 빈 문자열로 처리하도록 수정했습니다.

## 1.1.1

- 전송 payload의 `null` 문자열을 빈 문자열로 정규화하도록 수정했습니다.

## 1.1.0

- 전송 API를 반환값 없는 `SendEvent`, `SendReport`, `SendUx`로 변경했습니다.
- `ReporterResponse`를 제거하고 전송 실패를 Unity 로그에 기록하도록 변경했습니다.

## 1.0.1

- ReporterSettings 생성 메뉴를 `OOJJRS`에서 `Ore`로 변경했습니다.

## 1.0.0

- Unity 프로젝트를 `Packages/src` 기반 UPM 패키지 구조로 전환했습니다.
- `com.oojjrs.ore` 패키지 메타데이터와 `oojjrs.ore` 런타임 어셈블리를 추가했습니다.
- Reporter 보고서와 이벤트를 전송하는 비동기 클라이언트를 추가했습니다.
- Unity 정보와 사용자 정보, 스크린샷, 로그를 자동 구성하는 간편 전송 API를 추가했습니다.
- 설정 에셋을 지연 로드하는 `Ore` 정적 전송 API를 추가했습니다.
- 텍스트와 이미지 첨부파일 구성 및 서버 오류 응답 모델을 추가했습니다.
